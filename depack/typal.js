...
}, aa = (a, b = !1) => E(a, 2 + (b ? 1 : 0)), F = a => {
  ({callee:{caller:a}} = a);
  return a;
};
const {homedir:ba} = os;
const G = /\s+at.*(?:\(|\s)(.*)\)?/, ca = /^(?:(?:(?:node|(?:internal\/[\w/]*|.*node_modules\/(?:IGNORED_MODULES)\/.*)?\w+)\.js:\d+:\d+)|native)/, da = ba(), H = a => {
  ...
  }).filter(f => f.trim()).map(f => b ? f.replace(G, (g, h) => g.replace(h, h.replace(da, "~"))) : f).join("\n");
};
function ea(a, b, c = !1) {
  return function(d) {
    var e = F(arguments), {stack:f} = Error();
    const g = E(f, 2, !0), h = (f = d instanceof Error) ? d.message : d;
...
async function N(a, b, c) {
  ...
  return await new Promise((f, g) => {
    ...
    Array.isArray(b) ? (b.forEach((l, m) => {
      M(e, m);
    }), k = [...b, h]) : 1 < Array.from(arguments).length && (M(e, 0), k = [b, h]);
    a(...k);
  });
}
;const {join:O} = path;
async function ia(a, b) {
  b = b.map(async c => {
    const d = O(a, c);
    return {lstat:await N(A, d), path:d, relativePath:c};
  });
...
async function P(a) {
  ...
  if (!(await N(A, a)).isDirectory()) {
    throw a = Error("Path is not a directory"), a.code = "ENOTDIR", a;
  }
  var b = await N(B, a);
  b = await ia(a, b);
  a = b.filter(ja);
  b = b.filter(ka).reduce((c, d) => {
...
async function ma(a, b) {
  return await a.a.reduce(async(c, {re:d, replacement:e}) => {
    ...
    if ("string" == typeof e) {
      ...
      const f = [];
      let g;
      const h = c.replace(d, (k, ...l) => {
        g = Error();
        try {
          const m = e.call(a, k, ...l);
          m instanceof Promise && f.push(m);
          return m;
        } catch (m) {
          S(g, m);
        }
...
class na extends C {
  constructor(a, b) {
    super(b);
    this.a = (Array.isArray(a) ? a : [a]).filter(la);
  }
  async _transform(a, b, c) {
    try {
      const d = await ma(this, a);
      this.push(d);
      c();
    } catch (d) {
...
"u"), qa = V.source.replace(/\(\[/g, "(?:["), ra = new RegExp(`\\s*((?:${qa}\\s*)*)`, "u");
const sa = a => {
  const b = {};
...
const ta = (a, b) => {
  ...
  a = new RegExp(`<${a}${ra.source}?(?:${/\s*\/>/.source}|${(new RegExp(`>([\\s\\S]+?)?</${a}>`)).source})`, "gu");
  const c = [];
  for (var d; d = ta(a, b);) {
    if (!d.length) {
      continue;
...
function ua(a, b, {name:c, string:d, "boolean":e, opt:f, number:g, type:h, "default":k, closure:l}) {
  if (!c) {
...
;function xa(a, b, {name:c, type:d, desc:e, spread:f, noExpand:g, link:h, closure:k}, l) {
  if (!c) {
...
const Da = {re:/^\/\*\*? (documentary|typal) (.+?) \*\/\n(?:([^\n][\s\S]+?\n))?$/mg, replacement:async function(a, b, c) {
  ...
  try {
    ...
    this.emit("types", h);
    let l;
    d ? l = Aa(k, h) : e ? (l = Ba(h, g, this.namespaces) + "\n", g && this.emit("namespace", g)) : l = Ca(k, h);
    return `/* ${b} ${c} */\n${l}`;
  } catch (f) {
  ...
  b = b.map(c => Y(c));
  a = a.map(c => T(c, !1)).map(pa).join("");
  b = b.join("");
  return `${a}${b}`.replace(Ea, " * @typedef");
}, Ea = / \*\/\n\/\*\*\n \* @typedef/g;
...
const Ia = async(a, b = !1, c = !1, d = null) => {
  await Promise.all(a.map(async e => {
    ...
    g.end(f);
    f = await J(g);
    "-" == d ? console.log(f) : d ? await L(d, f) : await L(e, f);
  }));
};
(async() => {
  try {
