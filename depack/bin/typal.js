...
;function W(a, b, {name:c, type:d, desc:e, noToc:f, spread:g, noExpand:h, "import":k, link:l, closure:m, constructor:n, "extends":p, "interface":q, record:r}, t) {
  if (!c) {
...
`;
  ...
  const c = a.f.map(d => {
    ...
    e.push(` * @type {${d.optional ? Q(d.b) : d.b}}`);
    e = e.join("\n");
    e = `/**
${e}
 */
`;
    return e += S(`${a.c}.prototype`, d.name);
  });
  return [b, ...c].join("\n");
}
function La(a, b) {
...
const Oa = (a = [], b = !1) => `{ ${a.map(c => {
  const d = b ? c.b : c.type;
  let e = c.name, f = d;
  c.optional && !b ? e = `${c.name}?` : c.optional && b && (f = `(${Q(d)})`);
  return `${e}: ${f}`;
}).join(", ")} }`;
function Pa(a) {
  if ("object" != typeof a) {
...
const eb = {re:/^\/\*\*? (documentary|typal) (.+?) \*\/\n(?:([^\n][\s\S]+?\n))?$/mg, replacement:async function(a, b, c) {
  ...
  try {
    ...
    const n = await I(d);
    let {i:p = null, types:q, imports:r} = ab(n);
    q = q.filter(({c:w}) => k.includes(w) ? !1 : !0);
    r = r.filter(({c:w}) => k.includes(w) ? !1 : !0);
    this.emit("types", q);
    this.emit("types", r);
    let t;
    l ? t = bb(r, q, h) : m ? (t = cb(q, p, this.g) + "\n", p && this.emit("namespace", p)) : t = db(r, q);
    return `/* ${b} ${c} */\n${t}`;
  } catch (n) {
