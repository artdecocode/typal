...
class ua extends na {
  constructor(a) {
    ...
    const {F:f = G(!0), proxyError:g} = a || {}, h = (l, k) => f(k);
    super(b);
    this.a = [];
...
;function Ha(a, b, {name:c, string:d, "boolean":e, opt:f, number:g, type:h, "default":l, closure:k}) {
  ...
  a.type = ya({number:g, I:d, boolean:e, type:h});
  k ? a.c = k : a.c = a.type;
  void 0 !== l && (a.a = !0);
  a.a && (a.default = l);
  if (f || a.a) {
...
;function La(a, b, {name:c, type:d, desc:e, spread:f, noExpand:g, link:h, closure:l, constructor:k, "extends":m, "interface":n, record:p}, q) {
  if (!c) {
  ...
  !0 === p && (a.b = p);
  m && (a.extends = m);
  b && (a.f = T("prop", b).map(({content:r, u}) => {
    const C = new Ka;
    Ha(C, r, u);
    return C;
  }));
...
function W(a, b = !1) {
  const c = a.description ? ` ${a.description}` : "", d = !!a.extends, e = Na(a, b), f = [];
  if (a.g && b) {
    let g = ` * @typedef {${a.i}} ${a.name}${c}`;
    b && (g = Q(g));
    g = `/**
${g}
 */
`;
  ...
  d && (a = ` * @typedef {${a.extends} & ${V(a, b)}} ${b ? a.i : a.name}${c}`, b && (a = Q(a)), a = `/**
${a}
 */
`, f.push(a));
  f.push(e);
...
function Oa(a, b, c, d = "", e = !1, f = !1) {
  const g = a.description ? ` ${a.description}` : "", h = a.C ? X(a.f) : f ? a.i : a.name;
  c = `${d} * @param {${e ? "!" : ""}${h}} ${c ? `[${b}]` : b}${g}`;
  a = a.f && !a.s ? a.f.map(l => Ja(l, b, d, f)) : [];
  return [c, ...a].join("\n");
}
class Pa {
  constructor() {
    this.name = "";
    this.link = this.s = this.C = this.description = this.c = this.type = null;
    this.f = [];
    this.g = null;
  ...
  get i() {
    return `${this.j}${this.name}`;
  }
}
const X = (a = [], b = !1) => `{ ${a.map(c => {
...
class Z extends A {
  constructor(a, b) {
    super(b);
    this.j = (Array.isArray(a) ? a : [a]).filter(Qa);
    this.a = !1;
    this.s = b;
  }
  async replace(a, b) {
    const c = new Z(this.j, this.s);
    b && Object.assign(c, b);
    a = await Ra(c, a);
    c.a && this.brake();
  ...
  async reduce(a) {
    return await this.j.reduce(async(b, {re:c, replacement:d}) => {
      b = await b;
      if (this.a) {
        return b;
      }
...
function Xa(a, b = !0) {
  return ` * @typedef {import('${a.a}').${a.name}} ${b ? `${a.b}.${a.name}` : a.name}`;
}
class Ya {
  constructor() {
...
;const Za = a => {
  ...
  a = T("type", b).map(({content:g, u:h}) => {
    const l = new Pa;
    La(l, g, h, e);
    return l;
  });
  const f = T("import", b).map(({u:g}) => {
    const h = new Ya;
    Wa(h, g);
    return h;
  });
...
`);
  ...
  a = [...a.map(d => {
    {
      let e;
      d.c ? e = ` * @typedef {${d.c}}` : d.H || (e = ` * @typedef {${X(d.f, !0)}}`);
...
const cb = {re:/^\/\*\*? (documentary|typal) (.+?) \*\/\n(?:([^\n][\s\S]+?\n))?$/mg, replacement:async function(a, b, c) {
  const {o:d, w:e} = this.B;
  try {
    this.m("Detected type marker: %s", c);
    const f = await I(c), {g = null, types:h, imports:l} = Za(f);
    this.emit("types", h);
    let k;
    d ? k = $a(l, h) : e ? (k = ab(h, g, this.b) + "\n", g && this.emit("namespace", g)) : k = bb(l, h);
    return `/* ${b} ${c} */\n${k}`;
  } catch (f) {
    return this.m("(%s) Could not process typedef-js: %s", c, f.message), a;
  }
}}, bb = (a, b) => {
  b = b.map(c => W(c));
  a = a.map(c => Xa(c, !1)).map(za).join("");
  b = b.join("");
  return `${a}${b}`.replace(db, " * @typedef");
}, db = / \*\/\n\/\*\*\n \* @typedef/g;
const eb = {re:/( *) \* @param {(.+?)} (\[)?([^\s\]]+)\]?(?: .+)?((?:\n(?: +)\* @param {(?:.+?)} \[?\4\]?.*)*)/gm, replacement(a, b, c, d, e) {
  const {o:f} = this.B, g = /^!/.test(c), h = g ? c.replace(/^!/, "") : c;
  c = Object.values(this.types).find(({name:l, i:k}) => f ? k == h : l == h);
  return c ? Oa(c, e, d, b, g, f) : (this.m("Type %s not found", h), a);
}};
var gb = async() => {
  const {o:a = !1, w:b = !1, A:c} = {o:ca, w:da, A:y};
  var d = await L(z, x);
  let e;
  d.isFile() ? e = [x] : d.isDirectory() && (d = await N(x), e = O(d.content, x));
  await fb(e, a, b, c);
};
const fb = async(a, b = !1, c = !1, d = null) => {
  await Promise.all(a.map(async e => {
    var f = await I(e);
    const g = new Va([cb, eb], {o:b, w:c});
    g.m = console.error;
    g.end(f);
    f = await H(g);
...
const hb = a => {
  ...
  return void 0 !== b ? b : a;
}, Ga = /^ \* @prop {(.+?)} (\[)?(.+?)(?:=(["'])?(.+?)\4)?(?:])?(?: (.+?))?(?: Default `(.+?)`.)?$/gm, Fa = "type opt name quote defaultValue description Default".split(" "), Ta = new RegExp(`^ \\* @typedef {(.+?)} (.+?)(?: (.+))?\\n((?:${/ \* @prop(?:erty)? .+\n/.source})*)`, "gm"), ib = (a, b, c, d) => {
  d = d.length;
  a = a && "Object" != a ? ` type="${a}"` : "";
  c = c ? ` desc="${c}"` : "";
...
class jb extends A {
  ...
  _transform(a, b, c) {
    ...
    a = d && d.startsWith("import") ? kb(d, e) : ib(d, e, f, g);
    this.push(a);
    g.forEach(h => {
...
class lb extends A {
  ...
  _transform(a, b, c) {
    ...
    a = Ea(g).map(h => {
      ...
      n = Object.assign({}, l, {name:p, type:h}, k ? {defaultValue:hb(k)} : {}, m ? {l:hb(m)} : {}, n ? {optional:!0} : {});
      if (k || m) {
        k ? k !== m && void 0 !== n.l && (k = P(p, m, h), console.error("%s[%s] does not match Default `%s`.", e, k, n.l)) : (k = P(p, m, h), console.error("%s[%s] got from Default.", e, k)), n.default = "defaultValue" in n ? n.defaultValue : n.l, delete n.defaultValue, delete n.l;
      }
      return n;
    });
