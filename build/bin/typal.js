...
;const E = /\s+at.*(?:\(|\s)(.*)\)?/, F = /^(?:(?:(?:node|(?:internal\/[\w/]*|.*node_modules\/(?:IGNORED_MODULES)\/.*)?\w+)\.js:\d+:\d+)|native)/, G = x(), H = a => {
  const {pretty:b = !1, ignoredModules:c = ["pirates"]} = {}, d = c.join("|"), e = new RegExp(F.source.replace("IGNORED_MODULES", d));
  return a.replace(/\\/g, "/").split("\n").filter(f => {
    f = f.match(E);
    if (null === f || !f[1]) {
      return !0;
    ...
    f = f[1];
    return f.includes(".app/Contents/Resources/electron.asar") || f.includes(".app/Contents/Resources/default_app.asar") ? !1 : !e.test(f);
  }).filter(f => f.trim()).map(f => b ? f.replace(E, (g, h) => g.replace(h, h.replace(G, "~"))) : f).join("\n");
};
const aa = (a, b) => {
  b.once("error", c => {
    a.emit("error", c);
...
const I = async a => {
  var b = void 0 === b ? {} : b;
  ({c:a} = new ba(Object.assign({}, {rs:a}, b, {F:D(!0)})));
  return await a;
};
const {createReadStream:ca, createWriteStream:da, lstat:J, readdir:ea} = fs;
function K(a, b) {
  if (b > a - 2) {
...
class ma extends q {
  constructor(a, b) {
    super(b);
    this.a = (Array.isArray(a) ? a : [a]).filter(ka);
  }
...
"u"), oa = S.source.replace(/\(\[/g, "(?:["), pa = new RegExp(`\\s*((?:${oa}\\s*)*)`, "u");
const qa = a => {
  const b = {};
...
const ra = (a, b) => {
  ...
  a = new RegExp(`<${a}${pa.source}?(?:${/\s*\/>/.source}|${(new RegExp(`>([\\s\\S]+?)?</${a}>`)).source})`, "gu");
  const c = [];
  for (var d; d = ra(a, b);) {
    if (!d.length) {
      continue;
...
const sa = ({A:a, H:b, j:c, type:d}) => b ? "string" : a ? "number" : c ? "boolean" : d ? d : "*", ta = a => `/**
${a}
 */
...
function va(a, b, {name:c, string:d, "boolean":e, opt:f, number:g, type:h, "default":k, closure:m}) {
  if (!c) {
    throw Error("Property does not have a name.");
  }
  a.name = c;
  b && (a.description = b.trim());
  a.type = sa({A:g, H:d, j:e, type:h});
  m ? a.b = m : a.b = a.type;
  void 0 !== k && (a.a = !0);
  a.a && (a.c = k);
  if (f || a.a) {
...
;function ya(a, b, {name:c, type:d, desc:e, spread:f, noExpand:g, link:h, closure:k}, m) {
  if (!c) {
...
const Ea = {re:/^\/\*\*? (documentary|typal) (.+?) \*\/\n(?:([^\n][\s\S]+?\n))?$/mg, replacement:async function(a, b, c) {
  const {l:d, s:e} = this.w;
  try {
    this.i("Detected type marker: %s", c);
    const f = await P(c), {h:g, I:h, imports:k} = ua(f), m = h.map(({content:n, m:p}) => {
      const u = new za;
      ya(u, n, p, g);
      return u;
    });
    this.emit("types", m);
    let l;
    d ? l = Aa(k, m) : e ? (l = Ba(m, g, this.namespaces) + "\n", g && this.emit("namespace", g)) : l = Da(k, m);
    return `/* ${b} ${c} */\n${l}`;
  } catch (f) {
...
}({source:{D:!0}, closure:{B:"c", j:!0}, externs:{B:"e", j:!0}}), Ha = Z.source, Ia = Z.closure, Ja = Z.externs;
(async() => {
  try {
    return await La();
  } catch (a) {
