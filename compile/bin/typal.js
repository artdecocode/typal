...
;const N = (a, b, c, d) => {
  ...
  return `${a}=${b}`;
}, Ya = ({number:a, K:b, boolean:c, type:d}) => b ? "string" : a ? "number" : c ? "boolean" : d ? d : "*", Za = a => `${/[^\w\d._]/.test(a) ? `(${a})` : a}|undefined`, O = a => a ? `/**
${a}
 */
...
;function eb(a, b, {name:c, string:d, "boolean":e, opt:g, number:f, type:h}, k) {
  ...
  a.name = c;
  b && (a.description = R(b));
  b = Ya({number:f, K:d, boolean:e, type:h});
...
class rb {
  ...
  toTypeScriptFunction(a) {
    ...
    const {function:{args:b, return:c}} = this.parsed, d = b.map(g => a(g)).map((g, f) => {
      const {optional:h} = b[f];
      let {name:k = `arg${f}`, optional:l = h} = this.args[f] || {};
      return `${`${k}${l ? "?" : ""}`}: ${g}`;
    }).join(", "), e = c ? a(c) : "?";
    return `(${d}) => ${e}`;
  }
...
function wb(a) {
  ...
  const c = a.properties.reduce((d, e) => {
    d.push(e);
    const g = e.o.map(f => qb(e, f));
    d.push(...g);
    return d;
  }, []).filter(({isConstructor:d}) => !d).map(d => {
    let e = pb(d);
    e = O(e);
    e += Q(`${a.fullName}${d.J ? "" : ".prototype"}`, d.name);
    return e += ob(d);
  });
  return [b, ...c].join("\n");
}
...
function yb(a, b = !1, c = !1, d = b) {
  d = ` * @typedef {${(b ? a.closureType : a.type) || a.F()}}${` ${xb(a, d)}${a.h}`}`;
  a = (a.properties ? a.properties.reduce((e, g) => {
    if (g.b) {
      return e;
    }
    e.push(g);
    const f = g.o.map(h => qb(g, h));
    e.push(...f);
    return e;
  }, []) : []).map(e => lb(e, b));
  a = [d, ...a].join("\n");
...
class Y {
  ...
  b(a, {name:b, type:c, desc:d, noToc:e, spread:g, noExpand:f, link:h, closure:k, constructor:l, "extends":m, "interface":n, record:p}, q, r = null) {
    ...
    if (a) {
      ...
      a = T(["function", "fn", "static"], a).map(({content:t, props:v, tag:E}) => {
        E = "static" == E;
        const {H:Lb, C:ya} = gb(t, r), {async:za, "void":Mb, "return":Nb = Mb ? "void" : "", ...Aa} = v;
        ({args:v = ""} = v);
        v || (v = ya.map(({D:Ob}) => Ob).join(","));
        t = Nb.replace(/\n\s*/g, " ");
  ...
  f(a = !1, b = !1, c = a) {
    const d = !!this.extends, e = yb(this, a, b, c), g = [];
    if (this.namespace && a) {
    ...
    d && (c = ` * @typedef {${this.extends} & ${xb(this, c)}} ${c ? this.fullName : this.name}${this.h}`, a && !b && (c = P(c)), c = O(c), g.push(c));
    g.push(e);
    return g.join("");
  }
  get P() {
  ...
  B(a, b, c, d, e = !1) {
    ...
    const f = this.spread ? zb(this.properties) : e ? this.fullName : this.name;
    b = `${c || ""} * @param {${g}${f}} ${b ? `[${a}]` : a}${d}`;
    g = this.properties && !this.noExpand ? this.properties.map(h => h.B(a, c, e)) : [];
    return [b, ...g].join("\n");
  }
  toMarkdown(a = [], b = {}) {
    const {flatten:c, details:d = []} = b, e = d.includes(this.name);
    var g = this.type ? `\`${this.type}\`` : "", f = g, h = !1;
    this.link ? f = `[${g}](${this.link})` : !this.import && this.type && (f = X(a, this.type, b), h = f != this.type, f = Ab(f, h));
    g = Ab(this.fullName);
...
;const Fb = a => {
  if (a.args && a.args.length) {
    var b = `function(${a.args.map(({D:d}) => d).join(", ")}): ${a.fullName}`, c = new rb(a.args);
    c.isConstructor = !0;
  ...
  a = T(["type", "interface", "constructor", "method", "import"], b).reduce((f, {content:h, props:k, tag:l}) => {
    ...
    switch(l) {
      ...
      case "method":
        ...
        break;
      case "import":
        q = new Bb, q.b(h, k, k.ns || k.from, void 0), g.push(q);
    }
    return f;
  }, []);
  return {namespace:d, types:a, imports:g};
}, Ib = (a, b, c, d = !1) => {
  const e = d ? new Eb : new Y, g = a.search(/<(prop|function|fn|static) /);
  let f = "", h = a;
  1 != g && (f = a.slice(0, g), h = a.slice(g));
  const {C:k, H:l} = gb(f, void 0);
  ...
  let d, e, g;
  try {
    ({namespace:d = null, types:e, imports:g} = Hb(c));
  } catch (f) {
...
const Kb = (a, b, c) => {
  ...
  a = [...a.map(e => {
    {
      let g;
      e.closureType ? g = ` * @typedef {${e.closureType}}` : e.G || (g = ` * @typedef {${zb(e.properties, !0)}}`);
...
function Tb(a, b, c, d, e, g, f) {
  ...
  return !c || c instanceof Db.Import ? a : c.B(e, d, b, k.nullable, h);
}
const Z = (a, b, c, d, e) => {
  if (a) {
    var g = a.name;
...
class ec extends A {
  ...
  _transform([, a, b, c, d], e, g) {
    d = S($b, d, ac).map(f => {
      ...
      f = {...p, name:m, type:n, ...h ? {defaultValue:Zb(h)} : {}, ...k ? {u:Zb(k)} : {}, ...l ? {optional:!0} : {}};
      if (h || k) {
        if (h) {
...
class ic extends M {
  constructor(a, b) {
    super([{re:hc, async replacement(c, d, e, g) {
      const f = g.split(".");
      let h, k;
      if ("methodType" == e) {
        h = g;
